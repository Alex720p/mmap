...
bool mmap::map_dll(const char* process, const char* dll) {
	...
	try {
		...
		this->fix_relocations(buffer.get(), image.get(), base, pe_header, section_header, image_delta);
		std::printf("[+] Fixed relocations \n");

		this->resolve_imports(buffer.get(), image.get(), base, pe_header, section_header);
		std::printf("[+] Fixed relocations \n");

		this->write_buffer_to_target_process(buffer.get(), image.get(), base, pe_header, section_header);
		std::printf("[+] Dll written into target process memory \n");

		this->call_tls_callbacks(buffer.get(), image.get(), base, pe_header, section_header, image_delta);
		std::printf("[+] Called DLL_PROCESS_ATTACH TLS callbacks \n");

		this->call_dll_main(buffer.get(), image.get(), base, pe_header, section_header);
...
bool mmap::fix_relocations(char* buffer, char* image, std::uintptr_t base, IMAGE_NT_HEADERS* pe_header, IMAGE_SECTION_HEADER* section_header, int image_delta) {
	...
	while (offset_to_block_start < base_reloc_dir.Size) {
		...
		for (std::size_t j = 0; j < num_of_entries; j++) {
			...
			*reinterpret_cast<std::uintptr_t*>(page + offset) += image_delta;
		}
...
bool mmap::resolve_imports(char* buffer, char* image, std::uintptr_t base, IMAGE_NT_HEADERS* pe_header, IMAGE_SECTION_HEADER* section_header) {
	//resolve imports
	//TODO: test if bound == true works
	...
	//looks for module in target process: if found -> get base addr, not found -> inject shellcode to load it into target and read base addr store after shellcode : resolve all locally after for both
	while (import_dir.Size > sizeof(IMAGE_IMPORT_DESCRIPTOR) + (reinterpret_cast<std::uintptr_t>(import_descriptor) - import_desc_start_address)) { //+sizeof since last entry is empty
		...
		HMODULE fn_module_target = memory.get_module(module_name_wide).hModule; //TODO: recursively remap the dependency
		if (!fn_module_target) {
			/* Shellcode:
			...
			*/
			...
			if (!memory.write_memory_with_size(dll_name_base, (void*)module_name_ptr, lstrlenA(module_name_ptr) + 1)) //only c style cast works ????
		...
		HMODULE fn_module = LoadLibraryA(module_name_ptr);
		if (!fn_module || !fn_module_target)
			throw std::runtime_error("Failed to get import dependency " + module_name + " base address");

		std::uintptr_t fn_module_delta = reinterpret_cast<long>(fn_module_target) - reinterpret_cast<long>(fn_module);

		//TODO: change the types here to x64
		...
		while (*ilt_entry != 0) {
			...
			if (*ilt_entry & IMAGE_ORDINAL_FLAG32) { //import by ordinal
				std::uintptr_t fn_ordinal = IMAGE_ORDINAL64(*ilt_entry);
				fn_address = reinterpret_cast<std::uintptr_t>(GetProcAddress(fn_module, MAKEINTRESOURCEA(fn_ordinal)));
			} else { //import by name
				...
				fn_address = reinterpret_cast<std::uintptr_t>(GetProcAddress(fn_module, fn_name));
			}

			*iat_entry = fn_address + fn_module_delta;
...
bool mmap::write_buffer_to_target_process(char* buffer, char* image, std::uintptr_t base, IMAGE_NT_HEADERS* pe_header, IMAGE_SECTION_HEADER* section_header) {
...
bool mmap::call_tls_callbacks(char* buffer, char* image, std::uintptr_t base, IMAGE_NT_HEADERS* pe_header, IMAGE_SECTION_HEADER* section_header, std::size_t image_delta) {
	...
	*/
	...
	if (tls_dir_entry.Size) {
		...
		std::uintptr_t* callbacks = reinterpret_cast<std::uintptr_t*>(image - pe_header->OptionalHeader.ImageBase + tls_dir->AddressOfCallBacks); //ptr the the first elem of an array of callbacks

		while (*callbacks) {
			std::uintptr_t callback_addr_in_target = *callbacks + image_delta;
...
bool mmap::call_dll_main(char* buffer, char* image, std::uintptr_t base, IMAGE_NT_HEADERS* pe_header, IMAGE_SECTION_HEADER* section_header) {
	/* Shellcode:
